...
void IRAM_ATTR WWVBTimeSignalSource::PrepareMinute(time_t t)
{
  struct tm breakdown;
  struct tm tomorrow;
  time_t tomorrow_t;
...
WWVBTimeSignalSource::GetModulationForSecond(int sec)
{
  if (sec == 0 || sec % 10 == 9 || sec > 59)
    return (const SecondModulation) {(const ModulationDuration) {MOD_LOW, 800, 'M'}, (const ModulationDuration) {MOD_HIGH, 0}};
  const bool bit = time_bits_ & (1LL << (59 - sec));
  return (const SecondModulation) {(const ModulationDuration) {MOD_LOW, bit ? 500 : 200, bit ? '0' : '1'}, (const ModulationDuration) {MOD_HIGH, 0}};
}
